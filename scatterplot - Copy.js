...
var drivers = []
var driverPromise = d3.csv("data/drivers.csv", driverConverter).then(function (data) {
...
var driverPromise = d3.csv("data/drivers.csv", driverConverter)
...
d3.csv("data/sampleData.csv", rowConverter).then(function (data) {
    ...
    for(let i = 0; i < data.length; i++){
        let currDriverID = data[i]["driverid"];
        
        for(let x = 0; x < drivers.length; x++){
            if(currDriverID == drivers[x]['driverId']){
                console.log("match");
                if(!color_domain.includes(drivers[x]['teamId'])){
                    color_domain.push(drivers[x]['teamId']);
                    //console.log(drivers[x]['teamId'])
                }
            }
        }
        
        /*
        for(let constructorlength = 0; constructorlength < constructors.length; constructorlength++){
            if constructors["teamId"] ==
        }
        */
    }
